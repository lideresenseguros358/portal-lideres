-- =====================================================
-- ACTUALIZACIÓN MASIVA BROKERS - EJECUTAR EN SUPABASE
-- =====================================================

-- PASO 0: Habilitar extensión unaccent
CREATE EXTENSION IF NOT EXISTS unaccent;

-- PASO 1: Crear funciones helper
CREATE OR REPLACE FUNCTION clean_account(account TEXT) RETURNS TEXT AS $$
BEGIN RETURN CASE WHEN account IS NULL OR TRIM(account) = '' THEN NULL
    ELSE REGEXP_REPLACE(account, '[^0-9]', '', 'g') END;
END; $$ LANGUAGE plpgsql IMMUTABLE;

CREATE OR REPLACE FUNCTION get_bank_code(bank TEXT) RETURNS TEXT AS $$
BEGIN RETURN CASE 
    WHEN bank ILIKE '%GENERAL%' THEN '71' WHEN bank ILIKE '%BANISTMO%' THEN '22'
    WHEN bank ILIKE '%BAC%' THEN '45' WHEN bank ILIKE '%GLOBAL%' THEN '41'
    WHEN bank ILIKE '%SCOTI%' OR bank ILIKE '%NOVA%' THEN '50' WHEN bank ILIKE '%CAJA%' THEN '06'
    WHEN bank ILIKE '%CREDICORP%' THEN '47' ELSE NULL END;
END; $$ LANGUAGE plpgsql IMMUTABLE;
...
-- PASO 2: Crear tabla temporal con datos CSV
...
BEGIN
    FOR rec IN SELECT * FROM temp_broker_data LOOP
        UPDATE brokers b SET 
            ...
            nombre_completo = UPPER(UNACCENT(LEFT(rec.titular, 22))),
            beneficiary_name = UPPER(UNACCENT(LEFT(rec.titular, 22))),
...
SELECT 
    b.name, p.email, b.national_id, b.phone,
    b.bank_route, ab.bank_name,
    b.bank_account_no, b.tipo_cuenta, act.name as tipo_nombre,
    b.nombre_completo, b.beneficiary_name, b.percent_default,
    CASE WHEN b.bank_route IS NOT NULL AND b.bank_account_no IS NOT NULL 
         AND b.tipo_cuenta IS NOT NULL AND b.nombre_completo IS NOT NULL 
         THEN '✅' ELSE '⚠️' END as ach_ok
FROM brokers b
