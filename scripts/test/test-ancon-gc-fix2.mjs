/**
 * Fix GuardarCliente v2 — avoid special chars in prov_residencia
 * Use code '008' but also try 'PANAMA' without accent
 * Then immediately do full emission in same token
 */
...
async function main() {
  console.log('═══ FIX GUARDAR CLIENTE + FULL EMISSION ═══\n');
  ...
  // GuardarCliente — use code '008' for prov_residencia, no accented chars
  ...
  console.log(`\nGuardarCliente OK: ${gcOk}`);
  if (Array.isArray(gc) && gc[0]) console.log(`  cod_cliente: ${gc[0].cod_cliente}, Mensaje: ${gc[0].Mensaje}`);

  if (!gcOk) {
    console.log('\nGuardarCliente failed, stopping.');
    return;
  }
  ...
  const emitRes = await soap('EMIT', 'EmitirDatos', {
    ...
    no_chasis: 'JTDKN3DU5A0000009', nombre_conductor: 'JUAN',
    apellido_conductor: 'PEREZ', sexo_conductor: 'M', placa: '000000',
    puertas: '4', pasajeros: '5', cilindros: '4',
    vin: 'JTDKN3DU5A0000009', no_motor: '1NZ0000009', ano: '2025',
    ...
    nacionalidad: 'PANAMA', pep: '002', ocupacion: '001', profesion: '1',
