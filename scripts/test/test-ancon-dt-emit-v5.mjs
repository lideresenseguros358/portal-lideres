/**
 * ANCON DT Emission v5 — Proper GuardarCliente flow
 * 1. Token → 2. Estandar → 3. TipoCliente catalog → 4. GuardarCliente(contratante) →
 * 5. ClienteIgualContratante → 6. GenerarNodocumento → 7. EmitirDatos → 8. ImpresionPoliza
 */
...
function decode(t) { return t.replace(/&quot;/g,'"').replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&apos;/g,"'").replace(/&ntilde;/g,'ñ').replace(/&oacute;/g,'ó').replace(/&#039;/g,"'"); }
...
async function main() {
  console.log('═══ ANCON DT EMISSION v5 — PROPER CLIENT REGISTRATION ═══\n');
  ...
  // 2. Fetch TipoCliente and TipoIdentificacion
  console.log('── Catalogs ──');
  const tcRes = await soap('TC', 'TipoCliente', { token });
  const tiRes = await soap('TI', 'TipoIdentificacion', { token });
  const acRes = await soap('AC', 'ListaAseguradoContratante', { token });
  console.log('');
  ...
  // 4. GuardarCliente — figura='contratante' per docs page 26
  console.log('── STEP 3: GuardarCliente (figura=contratante) ──');
  ...
  const gcOk = Array.isArray(gc) && gc[0]?.Mensaje?.includes('exitosa');
  console.log(`  GuardarCliente OK: ${gcOk}\n`);
