/**
 * ANCON Full Flow v6 — SINGLE TOKEN, sequential calls, no token refresh
 * 1. Token
 * 2. Listaproductos (see all SOBAT plans)
 * 3. Estandar DT cotización  
 * 4. GuardarCliente with figura='1' and correct cod_producto
 * 5. ClienteIgualContratante
 * 6. GenerarNodocumento
 * 7. EmitirDatos
 * 8. ImpresionPoliza
 */
...
async function soap(label, method, params) {
  ...
  const short = JSON.stringify(parsed).substring(0, 250);
  const isErr = typeof parsed === 'string' && parsed.includes('Token Inactivo');
  console.log(`  [${label}] ${method} (${elapsed}ms) ${isErr ? '❌' : '✓'} → ${short}`);
  return parsed;
}
...
async function main() {
  console.log('═══ ANCON FULL FLOW v6 — SINGLE TOKEN ═══\n');
  ...
  // 2. Products — show SOBAT plans
  const prods = await soap('2-PRODS', 'Listaproductos', { token });
  if (Array.isArray(prods)) {
    console.log('\n  === ALL PRODUCTS ===');
    for (const p of prods) {
      console.log(`    ${p.codigo_producto} | ${p.codigo_ramo}/${p.codigo_subramo} | ${p.nombre_ramo} > ${p.nombre_subramo} | ${p.nombre_producto}`);
    }
    console.log('');
  }

  // 3. Estandar DT cotización
  const cotRes = await soap('3-COT', 'Estandar', {
  ...
  // 4. GuardarCliente — figura='1', cod_producto='07159'
  // With SINGLE token, this should work now
  const gc = await soap('4-GC', 'GuardarCliente', {
    tipo_persona: 'N',
    cod_producto: '07159',
    pasaporte: '',
  ...
  const gcOk = Array.isArray(gc) && gc[0]?.Mensaje?.toLowerCase().includes('exitosa');
  console.log(`  GuardarCliente success: ${gcOk}`);
  if (Array.isArray(gc) && gc[0]?.cod_cliente) {
    console.log(`  cod_cliente: ${gc[0].cod_cliente}`);
  }

  // 5. ClienteIgualContratante
  const cic = await soap('5-CIC', 'ClienteIgualContratante', { token, no_cotizacion: noCot, respuesta: '1' });

  // 6. GenerarNodocumento
  const genRes = await soap('6-GENDOC', 'GenerarNodocumento', {
  ...
  const emitRes = await soap('7-EMIT', 'EmitirDatos', {
    ...
    no_chasis: 'JTDKN3DU5A0000008', nombre_conductor: 'JUAN',
    apellido_conductor: 'PEREZ', sexo_conductor: 'M', placa: '000000',
    puertas: '4', pasajeros: '5', cilindros: '4',
    vin: 'JTDKN3DU5A0000008', no_motor: '1NZ0000008', ano: '2025',
    ...
    nacionalidad: 'PANAMÁ', pep: '002', ocupacion: '001', profesion: '1',
  ...
  if (emitOk) {
    console.log('\n  🎉 EMISIÓN DT EXITOSA!');
    await soap('8-PRINT', 'ImpresionPoliza', { token, no_poliza: poliza });
  } else {
    console.log(`\n  ❌ EMISIÓN FALLIDA: ${emitErr}`);
  }
}
