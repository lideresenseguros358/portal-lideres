-- =====================================================
-- SOLUCIÓN DEFINITIVA: Trigger preliminar corregido
-- =====================================================
-- EJECUTAR TODO ESTE ARCHIVO EN SUPABASE SQL EDITOR
-- =====================================================

-- 1. BORRAR trigger y función existentes
DROP TRIGGER IF EXISTS trigger_migrate_temp_client ON temp_client_import;
DROP FUNCTION IF EXISTS check_temp_client_complete() CASCADE;
DROP FUNCTION IF EXISTS migrate_temp_client_to_production(UUID) CASCADE;

-- 2. FUNCIÓN DE VALIDACIÓN (sin cambios)
...
-- 3. FUNCIÓN DE MIGRACIÓN (CORREGIDA)
CREATE OR REPLACE FUNCTION migrate_temp_client_to_production(temp_id UUID)
RETURNS void AS $$
DECLARE
  temp_record RECORD;
  new_client_id UUID;          -- ✅ Nombre correcto
  new_policy_id UUID;          -- ✅ Nombre correcto
...
BEGIN
  ...
  -- Crear póliza CON CAST DE STATUS
  INSERT INTO policies (
    ...
    new_client_id,                                                              -- ✅ Variable correcta
    temp_record.broker_id,
    ...
    COALESCE(UPPER(temp_record.status), 'ACTIVA')::policy_status_enum         -- ✅ CAST a enum
  )
...
-- 5. VERIFICACIÓN
SELECT 'Trigger y funciones recreados correctamente' AS status;
