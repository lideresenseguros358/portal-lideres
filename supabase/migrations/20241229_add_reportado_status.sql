...
-- Agregar nuevo constraint con REPORTADO (sin OK_CONCILIADO)
ALTER TABLE public.bank_transfers_comm
ADD CONSTRAINT bank_transfers_comm_status_check 
CHECK (status IN ('SIN_CLASIFICAR', 'PENDIENTE', 'REPORTADO', 'PAGADO'));

-- Comentario explicativo
COMMENT ON CONSTRAINT bank_transfers_comm_status_check ON public.bank_transfers_comm IS 
'Status válidos: SIN_CLASIFICAR (inicial), PENDIENTE (sin clasificar), REPORTADO (tipo reporte/bono), PAGADO (pagado en quincena)';
