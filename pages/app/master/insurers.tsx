...
export default function Insurers(){
  ...
  async function load(p=page, query=q){
    ...
    try{
      const r = await fetch(`/api/insurers?q=${encodeURIComponent(query)}&page=${p}&pageSize=${pageSize}`);
  ...
  return(
    <div className="wrap"><div className="card">
      ...
      <div className="top">
        ...
        <a className="btn" href={`/api/exports/insurers.csv?${qs}`}>CSV</a>
        <a className="btn" href={`/api/exports/insurers.xlsx?${qs}`}>XLSX</a>
        <a className="btn" href={`/api/exports/insurers.pdf?${qs}`}>PDF</a>
      </div>
      ...
      <table className="tbl">
        ...
        {!busy && rows.length===0 && <tr><td colSpan={3} style={{textAlign:'center',color:'#666'}}>Sin resultados</td></tr>}
        </tbody>
      </table>
