...
type Row = { id:string; name:string; email:string; role:string; active:boolean; created_at?:string; };
...
export default function MasterBrokers(){
  ...
  async function load(p=page, query=q){
    ...
    try{
      const r = await fetch(`/api/brokers?q=${encodeURIComponent(query)}&page=${p}&pageSize=${pageSize}`);
  ...
  return(
    <div className="wrap"><div className="card">
      ...
      <div className="top">
        <input placeholder="Buscar por nombre o email…" value={q}
               onChange={e=>setQ(e.target.value)}
               onKeyDown={e=> (e.key==='Enter') && load(1,q)} />
        <button onClick={()=>load(1,q)}>Buscar</button>
        <div className="spacer"/>
        <a className="btn" href={`/api/exports/brokers.csv?${qs}`}>CSV</a>
        <a className="btn" href={`/api/exports/brokers.xlsx?${qs}`}>XLSX</a>
        <a className="btn" href={`/api/exports/brokers.pdf?${qs}`}>PDF</a>
      </div>
      ...
      <table className="tbl">
        ...
        {!busy && rows.length===0 && <tr><td colSpan={5} style={{textAlign:'center',color:'#666'}}>Sin resultados</td></tr>}
        </tbody>
      </table>
