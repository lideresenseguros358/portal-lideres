...
export default function AgingBroker(){
  ...
  async function load(p=page, query=q){
    ...
    try{
      const r = await fetch(`/api/morosidad?me=1&q=${encodeURIComponent(query)}&page=${p}&pageSize=${pageSize}`, {
  ...
  const qs = `me=1&q=${encodeURIComponent(q)}&page=${page}&pageSize=${pageSize}`;

  return (
    <div className="wrap"><div className="card">
      <h2>Morosidad (mis clientes)</h2>
