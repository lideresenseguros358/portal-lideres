...
export default function IncendioWizard() {
    ...
    return () => {
      if (quoteLeadRef.current && !submittedRef.current) {
        trackQuoteFailed({
          ...
          lastStep: `incendio-paso-${stepRef.current}`,
  ...
  function goNext() {
    ...
    if (step === 1 && !quoteLeadRef.current) {
      const ref = `INCENDIO-LEAD-${Date.now()}`;
      ...
      trackQuoteCreated({
        ...
        ramo: 'INCENDIO',
        coverageType: 'Seguro de Incendio',
  ...
  const INCENDIO_STEPS: BreadcrumbStepDef[] = [
  ...
  const completedStepKeys = INCENDIO_STEPS.slice(0, step - 1).map(s => s.key);
  const stepTitles = INCENDIO_STEPS.map(s => s.label);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100" ref={formRef}>
      ...
      <EmissionBreadcrumb
        ...
        steps={INCENDIO_STEPS as any}
        onStepClick={(key: any) => {
          const idx = INCENDIO_STEPS.findIndex(s => s.key === key);
      ...
      <div className="py-8 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-bold text-[#010139] mb-1">Cotiza tu Seguro de Incendio</h1>
          ...
          <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-100 p-5 sm:p-7 mb-6">
            <div className="flex items-center gap-2 mb-5">
              {(() => { const Icon = INCENDIO_STEPS[step - 1]?.icon; return Icon ? <Icon className="text-xl text-[#8AAA19]" /> : null; })()}
