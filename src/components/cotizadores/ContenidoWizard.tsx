...
export default function ContenidoWizard() {
    ...
    return () => {
      if (quoteLeadRef.current && !submittedRef.current) {
        trackQuoteFailed({
          ...
          lastStep: `contenido-paso-${stepRef.current}`,
  ...
  function goNext() {
    ...
    if (step === 1 && !quoteLeadRef.current) {
      const ref = `CONTENIDO-LEAD-${Date.now()}`;
      ...
      trackQuoteCreated({
        ...
        ramo: 'CONTENIDO',
        coverageType: 'Seguro de Contenido / Hogar',
  ...
  async function handleSubmit() {
    // ── Broker path ───────────────────────────────────────────────────────────
    if (isBroker) {
      const errs = validateBrokerStep(brokerData, 'contenido');
      ...
      try {
        ...
        const res = await fetch('/api/cotizadores/broker-submit', {
          ...
          body: JSON.stringify({
            ...
            wizardData: {
              ...
              valorContenido: formatUSD(valorNum),
              articulosAltoValor: data.tieneArticulosAltoValor
                ? data.articulosAltoValor.map(a => ({ descripcion: a.descripcion, valor: formatUSD(parseCurrency(a.valor)) }))
                : [],
            },
            brokerData: {
              ...brokerData,
        ...
        console.error('[ContenidoWizard] Broker submit error:', err);
      ...
      <div className="space-y-5">
        ...
        <div className="bg-gradient-to-br from-[#010139]/5 to-[#8AAA19]/5 rounded-xl p-4 sm:p-5 border-2 border-[#8AAA19]/20">
          ...
          {(() => {
            ...
            return (
              <div className="mt-3 bg-[#8AAA19]/8 border border-[#8AAA19]/25 rounded-lg px-3 py-2">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            {step < EFFECTIVE_TOTAL ? (
              ...
              >
                {submitting ? (<><FaSpinner className="animate-spin" size={18} /> Enviando...</>) : (<><FaCheck size={16} /> {isBroker ? 'Enviar al especialista' : 'Enviar solicitud'}</>)}
              </button>
            )}
          </div>
