    ...
    <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10" ref={formRef}>
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-bold text-[#8AAA19]">Paso {step} de {TOTAL_STEPS}</span>
          <span className="text-xs text-gray-400">{stepTitles[step - 1]}</span>
        </div>
        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-[#010139] to-[#8AAA19] rounded-full transition-all duration-500 ease-out" style={{ width: `${(step / TOTAL_STEPS) * 100}%` }} />
        </div>
        <div className="flex justify-between mt-2">
          {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
            <button key={i} onClick={() => i + 1 < step && goToStep(i + 1)} disabled={i + 1 > step}
              className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full text-xs font-bold flex items-center justify-center transition-all ${i + 1 === step ? 'bg-[#010139] text-white shadow-lg scale-110' : i + 1 < step ? 'bg-[#8AAA19] text-white cursor-pointer hover:scale-105' : 'bg-gray-100 text-gray-400'}`}>
      ...
      <div className="flex items-center justify-between gap-4">
        {step > 1 ? (
          <button type="button" onClick={goBack} className="flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-colors">
            <FaArrowLeft size={12} /> Atrás
          </button>
        ) : <div />}
        {step < TOTAL_STEPS ? (
          <button type="button" onClick={goNext} className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-[#010139] to-[#8AAA19] hover:opacity-90 transition-opacity shadow-lg">
          ...
          <button type="button" onClick={handleSubmit} disabled={submitting}
            ...
            {submitting ? (<><FaSpinner className="animate-spin" size={14} /> Enviando...</>) : (<>Enviar solicitud <FaCheck size={12} /></>)}
          </button>
        )}
      </div>
    </div>
