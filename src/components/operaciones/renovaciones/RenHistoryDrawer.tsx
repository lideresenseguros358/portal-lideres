...
import { fmtDateTime } from './ren-helpers';
...
export default function RenHistoryDrawer({ open, onClose, caseId }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/operaciones/renewals?view=history&case_id=${caseId}`);
