...
import type { UrgencyStatus } from '@/types/operaciones.types';

function StatusBadge({ status }: { status: UrgencyStatus }) {
  const m: Record<UrgencyStatus, { bg: string; text: string }> = {
    ABIERTO: { bg: 'bg-red-100', text: 'text-red-800' },
    EN_ATENCION: { bg: 'bg-blue-100', text: 'text-blue-800' },
    RESUELTO: { bg: 'bg-green-100', text: 'text-green-800' },
    ESCALADO: { bg: 'bg-purple-100', text: 'text-purple-800' },
  };
