...
export default function IncendioConfirmacionPage() {
  ...
  useEffect(() => {
    setMounted(true);
    const raw = sessionStorage.getItem('incendioQuoteSubmission');
    if (raw) {
      try {
  ...
  const handleGoHome = () => {
    sessionStorage.removeItem('incendioQuoteSubmission');
    router.push('/cotizadores');
  };

  const handleNewQuote = () => {
    sessionStorage.removeItem('incendioQuoteSubmission');
    router.push('/cotizadores/incendio');
  };
      ...
      <div className="max-w-lg w-full">
        <div className="bg-white rounded-3xl shadow-2xl border-2 border-green-200 p-8 sm:p-10 text-center">
          ...
          <p className="text-base text-gray-600 mb-6">
            Tu solicitud de cotización de seguro de incendio ha sido recibida exitosamente.
          </p>
          ...
          {submission && (
            <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-xl p-5 mb-6 border-2 border-green-100 text-left space-y-3">
              ...
              <div className="flex justify-between items-center py-2 px-3 bg-white rounded-lg">
                <span className="text-xs text-gray-500">Valor estructura</span>
                <span className="text-sm font-bold text-[#8AAA19]">
                  ${submission.valorBien?.toLocaleString('en-US', { minimumFractionDigits: 0 })}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 px-3 bg-white rounded-lg">
