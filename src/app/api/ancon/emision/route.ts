...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ═══ STEP 0.5: GuardarCliente + ClienteIgualContratante ═══
    // Required per ANCON emission flow. EmitirDatos hits FK constraint unless a
    // client record exists first. Called as soft-fail: errors don't abort emission.
    log('0.5/4', 'Registrando cliente en ANCON (GuardarCliente)...');
    try {
      const nivelIngresoGC = (body as Record<string, string>).nivel_ingreso || (body as Record<string, string>).nivelIngreso || '';
      const INGRESO_CODE_MAP: Record<string, string> = {
        'menos_10k': '001', '10k_30k': '002', '30k_50k': '003', 'mas_50k': '004',
      };
      const montoIngreso = ['001', '002', '003', '004'].includes(nivelIngresoGC)
        ? nivelIngresoGC
        : (INGRESO_CODE_MAP[nivelIngresoGC] || '001');

      let fechaNacGC = fecha_nacimiento || '01/01/1990';
      if (fecha_nacimiento && !fecha_nacimiento.includes('/')) {
        const fparts = fecha_nacimiento.split('-');
        if (fparts.length === 3) fechaNacGC = `${fparts[2]}/${fparts[1]}/${fparts[0]}`;
      }

      // Parse cedula: "8-888-9999" → ced_prov=8, tomo=888, asiento=9999
      const cedulaParts = (cedula || '').split('-').filter(Boolean);

      const gcToken = await getFreshToken();
      const gcRaw = await rawSoap('GuardarCliente', {
        tipo_persona: tipo_de_cliente || 'N',
        cod_producto: '41',
        pasaporte: pasaporte || '',
        ...
        fecha_nac: fechaNacGC,
        sexo: sexo || 'M',
        presidencia: pais_residencia || 'PANAMÁ',
        nacionalidad: nacionalidad || 'PANAMÁ',
        direccion_laboral: (direccion || 'PANAMA').toUpperCase(),
        calle: '',
        casa: '',
        barriada: '',
        corregimiento: '',
        direccion_cobros: (direccion_cobros || direccion || 'PANAMA').toUpperCase(),
        ...
        email: email || '',
        apartado: '',
        ced_prov: cedulaParts[0] || '',
        ced_inicial: '',
        tomo: cedulaParts[1] || '',
        folio: '',
        asiento: cedulaParts[2] || '',
        ...
        cod_actividad: actividad_economica || '001',
        cod_clianiocon: '001',
        razon_social: '',
        token: gcToken,
      ...
      log('0.5/4', `GuardarCliente → ${JSON.stringify(gcRaw).substring(0, 300)}`);

      const cicRaw = await rawSoap('ClienteIgualContratante', {
        no_cotizacion: freshNoCotizacion,
        respuesta: '001',
        token: gcToken,
      });
      log('0.5/4', `ClienteIgualContratante → ${JSON.stringify(cicRaw).substring(0, 200)}`);
    } catch (gcErr: unknown) {
      log('0.5/4', `Error en registro de cliente (ignorado): ${gcErr instanceof Error ? gcErr.message : String(gcErr)}`);
    }
