/**
 * API Endpoint: Emitir póliza Contenido
 * POST /api/is/contenido/emitir
 * 
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Emitir póliza
    const result = await emitirContenido(
      {
        vIdPv,
    ...
    if (!clientePolicyResult.success) {
      console.error('[API IS Contenido Emitir] Error guardando cliente/póliza:', clientePolicyResult.error);
    ...
    console.error('[API IS Contenido Emitir] Error:', error);
    return NextResponse.json(
